...
for gz_release in gz_release_checks:
    ...
    for rep in reps:
        ...
        days_since_last_tag = round(
            (time.time() - repo.tags[newest_tag].commit.committed_date) / 86400
        )
        ...
        diff_text = repo.git.diff(
            repo.tags[newest_tag].commit, repo.commit("HEAD"), "--shortstat"
        )
        diff_text_clean = (
            diff_text.replace("deletions(-)", "")
            .replace("deletion(-)", "")
            .replace("insertions(+)", "")
            .replace("insertion(+)", "")
        ...
        entry = {
            rep: {
                "newest_tag": {
                    ...
                    "date": repo.tags[newest_tag].commit.committed_datetime.strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                },
                "newest_commit": {
                    ...
                    "date": repo.commit("HEAD").committed_datetime.strftime(
                        "%Y-%m-%d %H:%M"
                    ),
                },
                "stats": {
